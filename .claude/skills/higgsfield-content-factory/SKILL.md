...
description: >
  ...
  production cost. Also supports CLIENT MODE: small, ad-hoc orders (as few as 1-4 assets)
  produced for an external customer/agency client instead of a full owned-brand campaign —
  see "CLIENT MODE" section. Client Mode is also the entry point for orders handed off by
  an upstream email-intake agent. Trigger on a product image + content request, or phrases
  like 'create a campaign', 'build a content plan', 'make videos in Higgsfield', 'generate
  the image pack', 'schedule to Meta', 'run the content pipeline', 'make a test video for a
  client', 'small order', 'single video for [client]'.
---

# Higgsfield Content Factory

A 5-stage pipeline: Research → Plan → Generate → Publish → Report. Also runs in a
lightweight **Client Mode** (see below) for small per-customer orders.

---

## CLIENT MODE — small, per-customer orders (no full campaign)

> Use this mode instead of the full 5-stage pipeline whenever the ask is a **small, discrete
> order for/from a customer** rather than "build my brand's campaign" — e.g. "make 4 test
> videos for this lemonade," "one unboxing clip for client X," or any order that will later
> arrive pre-parsed from an **email-intake agent** (a separate agent that reads customer
> emails, extracts the brief, and hands it to this skill — see "Future: email-driven intake"
> below). Trigger phrases: "for a customer/client," "as a test," "just N videos/images,"
> "small order," or an Order Object (schema below) passed in directly.

### Why a separate mode

The full pipeline (Stages 1–5) is built for one brand running one big self-service
campaign: heavy trend research, an evenly-split 5-format mix, Meta Ads scheduling, a cost
report. A client-services shop doing many small orders for many different end-customers
needs something lighter: skip straight to generation, keep deliverables organized per
client/order, and don't force a 5-format split on a 4-asset test order.

### Order Object (the shared contract)

Every client order — whether typed in chat today or handed off by the future email agent —
is represented internally as this Order Object. Fill whatever fields are known; ask (via
AskUserQuestion buttons, per the button-driven rule) only for fields that are missing and
required to proceed.

```
order_id:        [auto: client-slug + YYYYMMDD + counter, e.g. "higg-lemonade-20260705-01"]
client_name:      [customer / brand this is FOR — may differ from "the account owner"]
contact:          [optional — email or name, useful once the email agent is wired in]
product:          { image: [file] | url: [string] | description: [text if neither] }
deliverables:      [
  { type: "video", format: one of the 5 UGC formats | "custom", count: N },
  { type: "image", style: "social"|"hero"|"with_people"|"without_people", count: N }
]
aspect_ratio:      [default 9:16 unless the platform implies otherwise]
brand_notes:       [free text: tone, must-avoid, variants/flavors to feature]
deadline:          [optional]
budget_note:       [optional — informs low/mid/high tier if the user cares about cost]
status:            draft | in_progress | delivered
output_dir:        /mnt/user-data/outputs/clients/[client-slug]/[order_id]/
```

### Client Mode flow

**1. Resolve the order.** If the user gave everything needed (product image/URL + how many
of what), skip straight to step 2. Otherwise ask ONE AskUserQuestion covering only the
missing pieces (still button-driven where the answer is a pick from a small set — e.g.
format mix; free text only for things like client name).

**2. Silent auto-detect.** Same routine as Stage 1 Step 1 (infer category, variants,
palette, demographic cues from the product image/URL) — no confirmation card, just a one-
line friendly summary.

**3. Pick formats for a SMALL order without forcing the 5-way split.** The 5-format even
split (Step C / "Default video split") is a campaign-scale default for N ≥ ~20. For small
orders (roughly N ≤ 8), instead:
   - If the user named specific formats/counts, use those exactly.
   - If they only gave a total count with no format preference, pick the **most
     representative 1–4 formats** for the product category from the Preset Relevance Map,
     and spread the count across just those — don't thin a 4-video order across 5 formats
     at "1 each, remainder somewhere." Prefer variety of *concept*, not forced format
     coverage: e.g. 4 videos for a beverage test → 1 UGC Entertainment, 1 Street Interview,
     1 Product Review, 1 ASMR (four different vibes) reads better than mechanically
     replicating the campaign-scale ratio.
   - Always still honor every Marketing Studio hard limit, avatar rule, no-on-screen-text
     rule, and the `mode` slug-mapping table from the main pipeline — Client Mode changes
     *scale and pacing*, not the generation rules.

**4. One permission gate, not five.** Small orders don't need Stage 3's per-format batch
gating — ask ONE AskUserQuestion before generating the whole order ("Ready to generate
these [N] assets for [client]?" — Yes / Change something / Cancel), then generate
everything in one pass. Still register the product (media_upload/media_confirm) and resolve
avatars/hooks/settings silently exactly as the main pipeline does.

**5. Deliver, don't campaign-report.** Save outputs to
`/mnt/user-data/outputs/clients/[client-slug]/[order_id]/` with descriptive filenames.
Present the results and ask (button): "Assets ready for [client]. What next?" —
"Done — deliver as-is (Recommended)" / "Regenerate one asset" / "Turn this into a full
campaign (Stages 2–5)" / "Start another client order". Stages 2, 4, 5 (HTML content plan,
Meta Ads scheduling, cost report) are opt-in extras for a client order, never automatic.

**6. Cost visibility (optional, lightweight).** If the user cares about per-order cost
(agency billing a client), a short 3-line credit/USD summary is enough — don't generate the
full Stage 5 HTML report unless asked.

### Future: email-driven intake (not yet wired up — placeholder for the next build phase)

The plan is for a **separate email-handling agent** to read incoming customer orders,
correspond with the customer, and hand a filled-in Order Object to this skill — at that
point Client Mode should be able to run with **zero chat back-and-forth** except the single
generation permission gate in step 4 (which the email agent would translate into an email
reply, not a chat button). Concretely, that means:
- The email agent owns all customer correspondence (confirmations, delivery, revisions).
- This skill only ever receives/produces Order Objects — it doesn't need to know how the
  order arrived (typed in chat today, or handed off programmatically later).
- Keep `client_name` / `contact` / `order_id` on every order now, even for manually-typed
  test orders, so the later automation is a drop-in, not a rewrite.
- Don't build the actual email polling/parsing here — that's a different agent's job and is
